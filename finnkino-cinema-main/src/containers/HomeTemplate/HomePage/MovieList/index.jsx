...
//Others
import "./style.scss";
import { SET_MOVIE_TYPE_NOW, SET_MOVIE_TYPE_SOON, GET_MOVIE_LIST_SUCCESS } from "@/store/constants/movieList";
import { actGetMovie } from "@/store/actions/movieList";

function MovieList() {
  useEffect(() => {
    dispatch(actGetMovie());
  }, []);
  ...
  const handleMovieType = () => {
    if (movieType === "now") {
      return (movieTypeList = movieList);
    ...
    <Box className="home__movie-list">
      <Typography align="center" className="home-list__btn-list">
        <Button
          ...
          onClick={() => dispatch({ type: GET_MOVIE_LIST_SUCCESS })}
        >
          Phim đang chiếu
        </Button>
        <Button
